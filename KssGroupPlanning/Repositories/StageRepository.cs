...
public class StageRepository : IStageRepository
{
	...
	{
		var stageEntities = await _dbcontext.Stages.AsNoTracking().OrderBy(c => c.Name).ToListAsync();
	...
	public async Task<Stage?> GetById(Guid id)
	{
		...
		var stageEntity = await _dbcontext.Stages.AsNoTracking().FirstOrDefaultAsync(c => c.Id == id);
	...
	{
		var stageEntity = await _dbcontext.Stages.AsNoTracking().FirstOrDefaultAsync(c => c.ProductId == productId);
		return Stage.Create(stageEntity.Id, stageEntity.Name,stageEntity.ProductId,stageEntity.Status, stageEntity.Date, stageEntity.CreateTime,stageEntity.UpdateTime);
	}
	...
	{
		var StageEntity = await _dbcontext.Stages.FirstOrDefaultAsync(s => s.Id == stage.Id)
			?? throw new Exception();
